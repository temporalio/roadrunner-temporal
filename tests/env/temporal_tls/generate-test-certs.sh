...
CERTS_DIR=$(pwd)/env/temporal_tls/certs
FILES_DIR=$(pwd)/env/temporal_tls/
mkdir "$CERTS_DIR"
...
openssl req -new -key "$CERTS_DIR"/cluster.key -out "$CERTS_DIR"/cluster.csr -config "$FILES_DIR"/cluster-cert.conf
openssl x509 -req -in "$CERTS_DIR"/cluster.csr -CA "$CERTS_DIR"/ca.cert -CAkey "$CERTS_DIR"/ca.key -CAcreateserial -out "$CERTS_DIR"/cluster.pem -days 365 -sha256 -extfile "$FILES_DIR"/cluster-cert.conf -extensions req_ext

# Generate a private key and a certificate for clients
openssl req -newkey rsa:4096 -nodes -keyout "$CERTS_DIR/client.key" -out "$CERTS_DIR/client.csr" -config "$FILES_DIR"/client-cert.conf
openssl x509 -req -in "$CERTS_DIR"/client.csr -CA "$CERTS_DIR"/ca.cert -CAkey "$CERTS_DIR"/ca.key -CAcreateserial -out "$CERTS_DIR"/client.pem -days 365 -sha256 -extfile "$FILES_DIR"/client-cert.conf -extensions req_ext
# Export to .pfx
