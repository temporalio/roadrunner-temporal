...
func Test_SingleActivityWorker_Replaced_NoPoolRestart(t *testing.T) {
	...
	// Give the watcher time to finish re-allocating the single replacement and the informer
	// to reflect it. A fixed wait (rather than polling the worker count) is deliberate: the
	// workflow completion above already gates on real recovery, and a count-based poll can
	// observe the replacement before it is fully ready, which then stalls pool teardown.
	time.Sleep(time.Second * 2)
