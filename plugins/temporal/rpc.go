...
func (r *Rpc) ExecuteWorkflow(inArg ExecuteWorkflow, reply *ExecuteWorkflowReply) error {
	ctx := context.Background()
	wr, err := r.client.ExecuteWorkflow(ctx, inArg.Options, nil, nil)
	if err != nil {
		return err
	}
	reply.WorkflowId = wr.GetID()
	reply.WorkflowRunId = wr.GetRunID()
	return nil
}
